function [output] = approxFlows_opt(x, links, nLinks, FD)
...
if links.destination(idxLink) == 1
    ...
    if in_red_phase
        %     denominator = 0.5 * (1-links.alpha_ij(idxLink))*(1+sign(dN)) + 0.5 * links.alpha_ji(idxLink)*(1-sign(dN));
...
% in_red_phase = links.h(idxLink) == 0;
%
% N_demand = 0.5*links.alpha_ji(idxLink)*r ...
%     + 0.5*(1-links.alpha_ij(idxLink))*g;
%
% if ~isnan(links.downstream_i(idxLink)) && links.destination(links.downstream_i(idxLink)) == 1
%     N_capacity = (g+r) * FD.qmax;
% else
%     N_capacity = g * boundaryCondition(idxLink).q_downstream;
% end
...
%
% if in_red_phase
%     dN = Delta_N - links.tau(idxLink) * 0.5 * links.alpha_ji(idxLink);
% elseif ~in_red_phase
%     dN = Delta_N - links.tau(idxLink) * 0.5 * (1-links.alpha_ij(idxLink));
% end
%
% end
